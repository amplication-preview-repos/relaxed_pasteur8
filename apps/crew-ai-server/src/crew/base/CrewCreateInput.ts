/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AgentCreateNestedManyWithoutCrewsInput } from "./AgentCreateNestedManyWithoutCrewsInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";
import { ResultCreateNestedManyWithoutCrewsInput } from "./ResultCreateNestedManyWithoutCrewsInput";

@InputType()
class CrewCreateInput {
  @ApiProperty({
    required: false,
    type: () => AgentCreateNestedManyWithoutCrewsInput,
  })
  @ValidateNested()
  @Type(() => AgentCreateNestedManyWithoutCrewsInput)
  @IsOptional()
  @Field(() => AgentCreateNestedManyWithoutCrewsInput, {
    nullable: true,
  })
  agents?: AgentCreateNestedManyWithoutCrewsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => ResultCreateNestedManyWithoutCrewsInput,
  })
  @ValidateNested()
  @Type(() => ResultCreateNestedManyWithoutCrewsInput)
  @IsOptional()
  @Field(() => ResultCreateNestedManyWithoutCrewsInput, {
    nullable: true,
  })
  results?: ResultCreateNestedManyWithoutCrewsInput;
}

export { CrewCreateInput as CrewCreateInput };
